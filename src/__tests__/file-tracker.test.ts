...
describe("FileTracker", () => {
	...
	describe("startTracking", () => {
		it("creates a new tracking entry for an agent", () => {
			tracker.startTracking("agent-1", "waypoint-1", "raid-1");

			const entry = tracker.getEntry("agent-1");
			expect(entry).toBeDefined();
			expect(entry?.agentId).toBe("agent-1");
			expect(entry?.waypointId).toBe("waypoint-1");
			expect(entry?.raidId).toBe("raid-1");
		...
		it("overwrites existing entry if tracking same agent", () => {
			...
			tracker.startTracking("agent-1", "waypoint-2", "raid-1");

			const entry = tracker.getEntry("agent-1");
			expect(entry?.waypointId).toBe("waypoint-2");
			expect(entry?.files).toEqual([]);
		});
	});
	...
	describe("detectConflicts", () => {
		it("detects conflicts when multiple agents modify same file", () => {
			tracker.startTracking("agent-1", "waypoint-1", "raid-1");
			tracker.startTracking("agent-2", "waypoint-2", "raid-1");
	...
	describe("clearRaid", () => {
		it("removes all entries for a specific raid", () => {
			tracker.startTracking("agent-1", "waypoint-1", "raid-1");
			tracker.startTracking("agent-2", "waypoint-2", "raid-1");
			tracker.startTracking("agent-3", "waypoint-3", "raid-2");
