...
/**
 * Generate review instructions for a completed task
 * This would be called after implementation, before commit
 */
export function generateReviewInstructions(passCount = 5): string {
	const annealing = new AnnealingReview({
		passesPerTemperature: 1,
		coolingRate: 0.35, // Faster cooling for fewer passes
	});

	const schedule = annealing.generateSchedule().slice(0, passCount);

	let instructions = "## Annealing Review\n\n";
	instructions += "Review this change through these lenses, starting broad and narrowing:\n\n";

	schedule.forEach((pass, i) => {
		instructions += `### Pass ${i + 1}\n`;
		instructions += `${annealing.formatPass(pass)}\n\n`;
	});
...
/**
 * Quick single-card review for lightweight mode
 */
export function quickReviewPrompt(): string {
	const annealing = new AnnealingReview();
	const pass = annealing.drawSingle();
	const modifier = annealing.drawOracleModifier();
	return `Review this change:\n- ${pass.prompt}\n- Also consider: "${modifier}"`;
}
