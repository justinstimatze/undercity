	...
	async runDiffTest(
		...
		try {
			if (useOllama) {
				...
				const diffApplied = testCase.expectedPattern ? testCase.expectedPattern.test(result.newContent || "") : true;
	...
	async runExperimentBatch(
		...
		for (let trial = 0; trial < trials; trial++) {
			for (const testCase of testCases) {
				...
				const localModel = (variant.parameters.localModel || "qwen2:1.5b") as LocalModel;

				// Run the test (model param only used when useOllama is true)
				const result = await this.runDiffTest(testCase, useOllama, localModel);
