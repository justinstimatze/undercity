...
import { createWriteStream, existsSync, mkdirSync, readFileSync, renameSync, WriteStream } from "node:fs";
...
export class DualLogger {
	...
	/**
	 * Write only to log file (for internal logging)
	 */
	...
	private rotateCurrentLog(raidId?: string): void {
		...
		try {
			...
			const archiveName = raidId
				? `raid-${raidId}-${timestamp}.log`
