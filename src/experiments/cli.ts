...
export class ExperimentCLI {
    ...
    try {
      switch (templateName.toLowerCase()) {
        ...
        default:
          ...
          console.log("Available templates: opus-vs-mixed, squad-composition, parallelism, context-size, auto-approval, comprehensive, ollama-vs-haiku, local-model-comparison, ollama-diff");
          return;
      }
  ...
  async runDiffExperiment(experimentId: string, trials?: number): Promise<void> {
    try {
      ...
      const isDiffExperiment = experiment.tags.includes("diff-generation") ||
