...
export class ExperimentCLI {
	...
	listExperiments(status?: ExperimentStatus): void {
		...
		for (const exp of filtered) {
			...
			console.log(id.padEnd(12) + name.padEnd(25) + status.padEnd(12) + variantCount.padEnd(10) + created);
		}
		console.log();
	}
	...
	showExperiment(experimentId: string): void {
			...
			if (metrics) {
				console.log(
					`     Success: ${(metrics.successRate * 100).toFixed(1)}% | ` +
						`Tokens: ${metrics.avgTokensPerTask.toFixed(0)} | ` +
		...
		if (analysis.significanceTests.length > 0) {
			...
			for (const test of analysis.significanceTests) {
				...
				console.log(
					`  ${significant} ${test.metric}: ${test.improvement.toFixed(1)}% improvement (p=${test.pValue.toFixed(4)})`,
				);
			}
			console.log();
		}
		...
		try {
			...
			if (analysis.significanceTests.length > 0) {
				...
				for (const test of analysis.significanceTests) {
					...
					console.log(
						`  ${test.metric}: ${test.improvement.toFixed(1)}% improvement (${status}, p=${test.pValue.toFixed(4)})`,
					);
				}
				console.log();
			}
			...
			for (const variant of experiment.variants) {
				...
				if (metrics && samples > 0) {
					console.log(
						`  ${variant.name}: ${(metrics.successRate * 100).toFixed(1)}% success, ` +
							`${metrics.avgTokensPerTask.toFixed(0)} tokens, ` +
