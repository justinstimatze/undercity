...
export class ExperimentFramework {
  ...
  /**
   * Assign a quest to an experiment variant using deterministic randomization
   */
  assignQuestToVariant(questId: string, experimentId: string): QuestAssignment | null {
    const storage = this.loadStorage();
    const experiment = storage.experiments[experimentId];
    ...
    // Check if quest is already assigned to this experiment
    const existingAssignment = storage.assignments.find(
      a => a.questId === questId && a.experimentId === experimentId
    );
    ...
    // Create deterministic seed from quest ID and experiment ID
    const seedString = `${questId}-${experimentId}`;
  ...
  /**
   * Get the variant parameters for a quest
   */
  getVariantParameters(questId: string, experimentId: string): VariantParameters | null {
    const assignment = this.getQuestAssignment(questId, experimentId);
  ...
  /**
   * Get quest assignment for a specific experiment
   */
  getQuestAssignment(questId: string, experimentId: string): QuestAssignment | null {
    const storage = this.loadStorage();
    return storage.assignments.find(
      a => a.questId === questId && a.experimentId === experimentId
    ) || null;
  }
  ...
  recordOutcome(
    ...
    if (!assignment) {
      throw new Error(`No assignment found for quest ${questId} in experiment ${experimentId}`);
    }
  ...
  /**
   * Get all active experiments that should receive quest assignments
   */
  getActiveExperiments(): Experiment[] {
    const storage = this.loadStorage();
    return Object.values(storage.experiments).filter(
  ...
  analyzeExperiment(experimentId: string): ExperimentAnalysis {
    ...
    for (const [variantId, variantOutcomes] of outcomesByVariant) {
      ...
      const diffGenTimes = diffOutcomes
        .map(o => o.diffMetrics?.diffGenerationTimeMs || 0)
        .filter(t => t > 0);

      variantMetrics[variantId] = {
        successRate: successCount / variantOutcomes.length,
        avgTokensPerQuest: totalTokens / variantOutcomes.length,
        ...
        avgDiffGenerationTimeMs: diffGenTimes.length > 0
          ? diffGenTimes.reduce((sum, t) => sum + t, 0) / diffGenTimes.length
          : undefined,
      };
    }
    ...
    for (const treatmentVariant of treatmentVariants) {
      ...
      significanceTests.push({
        ...
        improvement: (variantMetrics[treatmentVariant.id].successRate - variantMetrics[controlVariant.id].successRate) / variantMetrics[controlVariant.id].successRate * 100,
    ...
    if (!hasMinimumSamples) {
      ...
      winningVariant = treatmentVariants.find(v =>
        variantMetrics[v.id].successRate === bestTest.treatmentValue
      )?.id;
