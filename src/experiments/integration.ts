 ...
 *
 * Integrates the experimentation framework with the task execution system
 * to automatically assign tasks to experiments and track outcomes.
 */

import type { Task } from "../task.js";
import type { LoadoutConfiguration } from "../types.js";
import type { ExperimentFramework } from "./framework.js";
import type { VariantParameters } from "./types.js";

export class TaskExperimentIntegrator {
	constructor(private experimentFramework: ExperimentFramework) {}

	/**
	 * Get experiment assignments for a task and apply parameter overrides
	 */
	async processQuestForExperiments(
		...
		for (const experiment of activeExperiments) {
			const assignment = this.experimentFramework.assignTaskToVariant(task.id, experiment.id);
			...
			if (assignment) {
				...
				const variantParams = this.experimentFramework.getVariantParameters(task.id, experiment.id);
	...
	private applyVariantParameters(
		...
		if (parameters.agentsComposition) {
			modifiedLoadout.enabledAgentTypes = parameters.agentsComposition;
		}

		// Apply max squad size
		if (parameters.maxAgents !== undefined) {
			modifiedLoadout.maxAgents = parameters.maxAgents;
		}
	...
	/**
	 * Record task completion outcome for all assigned experiments
	 */
	async recordQuestOutcome(
		...
		for (const assignment of assignments) {
			try {
				this.experimentFramework.recordOutcome(assignment.experimentId, task.id, outcome);
			} catch (error) {
				console.warn(`Failed to record outcome for experiment ${assignment.experimentId}:`, error);
			}
		}
	}
		...
		for (const experiment of activeExperiments) {
			try {
				...
				if (analysis.recommendation === "stop_winner" && analysis.winningVariant) {
					...
					console.log(
						`Experiment "${experiment.name}" shows no significant effect. ` + `Consider stopping or redesigning.`,
					);
				} else if (analysis.status === "insufficient_data") {
					const minSamples = Object.values(analysis.sampleSizes).reduce((min, size) => Math.min(min, size), Infinity);
					const needed = experiment.targetSampleSize - minSamples;

					if (needed > 0) {
						console.log(`Experiment "${experiment.name}" needs ${needed} more samples per variant`);
					}
				}
			} catch (error) {
	...
	/**
	 * Check if a task should be included in experiments based on criteria
	 */
	shouldIncludeQuestInExperiments(task: Task): boolean {
		// Exclude already completed or failed tasks
		if (task.status === "complete" || task.status === "failed") {
			return false;
		}

		// Exclude test tasks or specific patterns
		if (task.objective.toLowerCase().includes("test") && task.objective.toLowerCase().includes("experiment")) {
			return false;
		}
	...
	generateExperimentReport(experimentId: string): string {
		...
		for (const variant of experiment.variants) {
			...
			if (metrics) {
				...
				report += `- Avg tokens: ${metrics.avgTokensPerTask.toFixed(0)}\n`;
